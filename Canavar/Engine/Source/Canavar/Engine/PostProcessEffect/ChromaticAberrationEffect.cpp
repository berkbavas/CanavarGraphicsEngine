...
Canavar::Engine::ChromaticAberrationEffect::ChromaticAberrationEffect()
{
    mShader = std::make_unique<Shader>("Chromatic Aberration Shader");
    mShader->AddPath(QOpenGLShader::Vertex, ":/Resources/Shaders/Quad.vert");
    mShader->AddPath(QOpenGLShader::Fragment, ":/Resources/Shaders/ChromaticAberration.frag");
    mShader->Initialize();

    mQuad = std::make_unique<Quad>();
}

void Canavar::Engine::ChromaticAberrationEffect::ApplyEffect(Framebuffer* pInput, Framebuffer* pOutput)
{
    pOutput->Bind();
